...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-Fussiongsi"
...
sed -i "s/$bdisplay/$displayid2=Ported\.by\.Fussion-Lab\.using\.ErfanTools/" $systemdir/system/build.prop
